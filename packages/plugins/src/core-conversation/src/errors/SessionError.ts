...
export class SessionError extends ConversationError {
  ...
  static alreadyExists(sessionId: string): SessionError {
    return new SessionError(`Session already exists: ${sessionId}`, sessionId, {
  ...
  static closed(sessionId: string): SessionError {
    return new SessionError(`Session is closed: ${sessionId}`, sessionId, {
  ...
  static expired(sessionId: string, ttl?: number): SessionError {
    return new SessionError(`Session expired: ${sessionId}`, sessionId, {
  ...
  static maxReached(max: number): SessionError {
    return new SessionError(`Maximum number of sessions reached: ${max}`, undefined, {
      max,
